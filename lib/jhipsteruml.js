...
if (Object.keys(argv).length < 3) {
  throw new buildException(
      ...
      "参数个数不正确，输入文件和数据库类型('sql', 'mongodb' or 'cassandra')必填.\n"
      + "Use the command 'genie-uml -help' to see the available commands.");
}
...
if (!argv.db && !isYoRcFilePresent()) {
  throw new buildException(
      ...
      + "Use the command 'genie-uml -help' to see the available options."
  );
} else {
